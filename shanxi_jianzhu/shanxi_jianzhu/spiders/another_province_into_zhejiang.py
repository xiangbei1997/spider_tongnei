...
class ShanxiJianzhuImformationSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for t in tr:
            ...
            print(self.bigurl + url, 'AAAAAAAAAAAAAAAAAAAA')
            yield Request(url=self.bigurl + url, callback=self.company_information,dont_filter=True)
        if self.index != 278:
            yield scrapy.Request(url=self.url + '?p=%s'% self.index, callback=self.parse)
