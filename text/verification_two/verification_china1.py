    ...
    def analog_drag(self):
        ...
        try:
            WebDriverWait(self.driver, 5, 0.5).until(
                EC.element_to_be_clickable((By.XPATH, '//div[@class="geetest_panel_box"]')))
            print("验证失败")
            return
        ...
        # 判断是否验证成功
        s = self.driver.find_elements_by_xpath('//a[@class="search_list_item db"]')
        self.set_url = set()
        if len(s) == 0:
            print("滑动解锁失败,继续尝试")
            self.analog_drag()
            ...
            company_name = self.driver.find_elements_by_xpath('//a[@class="search_list_item db"]/font[@color="red"]')
            print(company_name)
