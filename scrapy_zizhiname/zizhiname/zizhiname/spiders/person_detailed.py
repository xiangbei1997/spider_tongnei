...
class AllXinliangSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        # 提交form数据
        self.corporate_name = '厦门纵横集团通信发展有限公司'
        # 访问并携带参数
    ...
    def detailed_information(self, response):
        ...
        #             if td == '统一社会信用代码':
        #                 print(table_body)
        #                 self.company_zz['licenseNum'] = table_body
        #             elif td == '企业法定代表人':
        #                 self.company_zz['legalMan'] = table_body
        #             elif td == '企业登记注册类型':
        #                 self.company_zz['econType'] = table_body
        #             elif td == '企业注册属地':
        #                 self.company_zz['area'] = table_body
        #             elif td == '企业经营地址':
        #                 self.company_zz['address'] = table_body
        ...
        person = Selector(response=response).xpath('//ul[@class="tinyTab datas_tabs"]/li[@class="activeTinyTab"]/@data-url').extract_first()
        print(person)
    ...
    #                 text = a.xpath('./a/@onclick').extract_first()
    #                 # top.window.location.href='/dataservice/query/staff/staffDetail/001707270333279884'
    #                 person_url = text.split('top.window.location.href=\'')[1]
    #                 person_url = self.url + person_url
    #                 yield Request(url=person_url,callback=self.person)
    #
    #                 print(person_url)
    #
    #             # 如果是资质名称的话就获取去除\n\r\t
